...
public class TxtValidator {
    ...
    public boolean lenth(int min, int max){
    ...
    public boolean isInteger(int min, int max){
        if(!isInteger()) return false; //si el contenido no es un entero, return false
        int nro = Integer.parseInt(txt.getText());
        if(nro >= min && nro <= max) return true;
        return error("Debe ser un número entre " + min + " y " + max );
    }
